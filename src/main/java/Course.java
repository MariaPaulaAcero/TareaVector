...
public class Course {
    ...
    private Vector<Student> students1;
    ...
    public boolean registerL(Student student) {
        ...
        if (MAXIMUM_QUOTA > this.students1.size()) {
            for (int i = 0; i < this.students1.size() && !exists; i++) {
                if (this.students1.get(i).equals(student)) {
                    exists = true;
                }
            }

            if (!exists) {
                result = this.students1.add(student);
            }
        }
